...
public class Utils {
    ...
    public static List<String> getScoreboard() {
        ...
        try {
            ...
            scoreboardLines.add(stripColor(sidebarObjective.getDisplayName()));
            try {
    ...
    public static boolean aabbEquals(AxisAlignedBB aabb1, AxisAlignedBB aabb2) {
        if (aabb1 != null && aabb2 != null) {
            return aabb1.minX == aabb2.minX && aabb1.minY == aabb2.minY && aabb1.minZ == aabb2.minZ && aabb1.maxX == aabb2.maxX && aabb1.maxY == aabb2.maxY && aabb1.maxZ == aabb2.maxZ;
        }
        return false;
    }

    public static boolean aabbContains(List<AxisAlignedBB> aabbArray, AxisAlignedBB aabbSearch) {
        try {
            for (AxisAlignedBB aabb : aabbArray) {
                if (aabbEquals(aabb, aabbSearch)) {
                    return true;
                }
            }
        } catch (Exception ignored) {
    ...
    public static String getCurrentActionBar() {
        try {
            return stripColor((String) ReflectionHelper.findField(GuiIngame.class, "displayedActionBar", "recordPlaying").get(Minecraft.getMinecraft().ingameGUI));
